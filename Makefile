...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/hyperspeednetwork/hsnhub/types"
	godoc -http=:6060
...
test_ledger_mock:
	@go test -mod=readonly `go list github.com/hyperspeednetwork/hsnhub/crypto` -tags='cgo ledger test_ledger_mock'

test_ledger: test_ledger_mock
	@go test -mod=readonly -v `go list github.com/hyperspeednetwork/hsnhub/crypto` -tags='cgo ledger'
...
devdoc_init:
	docker run -it -v "$(CURDIR):/go/src/github.com/hyperspeednetwork/hsnhub" -w "/go/src/github.com/hyperspeednetwork/hsnhub" tendermint/devdoc echo
	# TODO make this safer
	$(call DEVDOC_SAVE)

devdoc:
	docker run -it -v "$(CURDIR):/go/src/github.com/hyperspeednetwork/hsnhub" -w "/go/src/github.com/hyperspeednetwork/hsnhub" devdoc:local bash
